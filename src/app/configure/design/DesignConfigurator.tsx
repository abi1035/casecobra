...
const DesignConfigurator = ({
  ...
  const {mutate:saveConfig}=useMutation({
    ...
    // Gets options from option-validators
    ...
    // Triggers when the image is being resized
    const [renderedDimension, setRenderedDimension] = useState({
      width: imageDimensions.width / 4,
      height: imageDimensions.height / 4,
    })
    
    // Triggers when the image is being dragged
    ...
    return(
        <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
            <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                ...
                // Gets the image poisition once the user stops
                onResizeStop={(_,__,ref,___,{x,y})=>{

                    // In 50px we are slicing away the px using slice and getting the 50
                    ...
                    // Triggers on resizing
                    setRenderedPosition({x,y})
                }}

                // Triggers when object is dragged across 
                onDragStop={(_,data)=>{
                    const {x,y}=data
                    setRenderedPosition({x,y})
                }}
            ...
            <div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
                <ScrollArea className="relative flex-1 overflow-auto">
                    {/* deleted class name: bg-gradient-to-t from-white */}
                    <div aria-hidden="true" className="absolute z-10 inset-x-0 bottom-0 h-12  pointer-events-none"/>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            {MODELS.options.map((model)=>(
                                ...
                                }}><CheckIcon className={cn("mr-2 h-4 w-4", model.label===options.model.label ? "opacity-100": "opacity-0")}/> {model.label}</DropdownMenuItem>
                            )
